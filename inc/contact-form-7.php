...
class Cf7forms_List extends WP_List_Table {
	
	public function __construct() {
		parent::__construct( [
			'singular' => __( 'CF7 Form', 'visitorsproof' ),
			'plural'   => __( 'CF7 Forms', 'visitorsproof' ),
	...
	public static function get_custom_notifications( $per_page = 5, $page_number = 1 ) {
		global $wpdb;
		$sql = "SELECT * FROM {$wpdb->prefix}" . VISITORS_PROOF_TABLE_NOTIFICATIONS . " WHERE is_custom = 2 ";
	...
	public static function record_count() {
		global $wpdb;
		$sql = "SELECT COUNT(*) FROM {$wpdb->prefix}" . VISITORS_PROOF_TABLE_NOTIFICATIONS . " WHERE is_custom = 2";
	...
	public function no_items() {
		_e( 'No CF7 Forms available', 'visitorsproof' );
	}
	...
	function column_icon( $item ) {
		...
		return $svg . '<br/>' . ($rules->type == 'Form' ? 'Form Page' : 'All Pages');
	}
	
	function column_content( $item ) {
		$delete_nonce = wp_create_nonce( 'vp_cf7_delete_custom_notification' );

		$cf7 = get_post($item['type']);
		$title = "<b>$cf7->post_title</b><br/>" . strip_tags($item['content']);
		...
		$actions = [
		    ...
		    'delete' => sprintf( '<a href="?page=%s&action=%s&notification_id=%s&_wpnonce=%s" onClick="return confirm(\'' . $confirmation_text . '?\');">Delete</a>', esc_attr( $_REQUEST['page'] ), 'delete', absint( $item['id'] ), $delete_nonce )
		];
	...
	public function prepare_items() {
		...
		$per_page     = $this->get_items_per_page( 'vp_cf7_per_page', 5 );
	...
	public function process_bulk_action() {
	    ...
	    $redirect_url = '?page=' . VISITORS_PROOF_PAGE_CF7 . '&success=1';
		...
		if ( 'delete' === $current_action ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'vp_cf7_delete_custom_notification' ) ) {
				die( 'Oops!' );
			}else {
